...
if(ctx){
  grafTemp = new Chart(ctx, {
    type: "line",
    data: { labels: [], datasets: [{ label: "°C", data: [], borderColor:"#1e90ff", backgroundColor:"rgba(30,144,255,0.2)", tension:0.3, fill:true }] },
...
async function obtenerDatos(){
  try {
    ...
    document.getElementById("windMaxBig").textContent = maxWindDia !== null ? maxWindDia.toFixed(1) + " km/h" : "--";
    ...
    ].forEach(b => { if(b.bar) b.bar.style.width = `${Math.min((b.val/maxRain)*100,100)}%`; });

    // =====================
    // Presión
    // =====================
    document.getElementById("press").textContent = inHgToHpa(data.pressure.relative?.value) !== "--"? inHgToHpa(data.pressure.relative.value).toFixed(1) : "--";
    ...
    document.getElementById("uv").textContent = uv !== undefined ? uv : "--";
    document.getElementById("solar").textContent = solar !== undefined ? solar : "--";
    ...
    // =====================
    document.getElementById("ultimaActualizacion").textContent = "Última actualización: "+new Date().toLocaleTimeString();
